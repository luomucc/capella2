...
public class RenameTool extends AbstractDiagramStep {
	 ...
	 */
	public RenameTool(DiagramContext context, String editLabelToolName,
			EObject renamedElement, String newName) {
		super(context);
	...
	protected void runTest() {
		SemanticEditingDomain editingDomain = (SemanticEditingDomain) getExecutionContext()
				.getExecutionManager().getEditingDomain();
		UndoRedoCapableEMFCommandFactory commandFactory = new UndoRedoCapableEMFCommandFactory(
				editingDomain);
		DRepresentationElement repElement = null;
		for (EObject obj : getExecutionContext().getDiagram().eContents()) {
