...
public class Service_RealizedExchangeItemsController extends AbstractMultipleSemanticFieldController {
  ...
  protected IBusinessQuery getReadOpenValuesQuery(EObject semanticElement) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement.eClass(), InformationPackage.Literals.OPERATION__OWNED_EXCHANGE_ITEM_REALIZATIONS);
  }
    ...
    if (lst instanceof Collection<?>) {
      for (Object obj : (Collection<?>) lst) {
        if (obj instanceof ExchangeItemRealization) {
          values.add(((ExchangeItemRealization) obj).getTargetElement());
        }
      }
    }
  ...
  protected void doAddOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ExchangeItemRealization link = InformationFactory.eINSTANCE.createExchangeItemRealization();
  ...
  protected void doRemoveOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ...
    for (EObject obj : (List<EObject>) semanticElement.eGet(semanticFeature)) {
      if ((obj instanceof ExchangeItemRealization)
        && ((ExchangeItemRealization) obj).getTargetElement().equals(object))
      {
        linkToRemove = obj;
      }
    }
