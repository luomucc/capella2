...
public class GetAvailable_StateTransition_StateTransitionRealizations extends AbstractQuery {
	...
	public List<CapellaElement> getAvailableElements(CapellaElement element) {
		...
		if (element instanceof StateTransition) {
			availableElements.addAll(getRule_MQRY_State_AvailableTransitions_11((StateTransition) element));
		}
	 ...
	 * @param state
	 */
	private List<CapellaElement> getRule_MQRY_State_AvailableTransitions_11(StateTransition state) {
	 ...
	 * @return
	 */
	private List<CapellaElement> getElementsFromBlockArchitecture(BlockArchitecture arch, StateTransition state) {
		List<CapellaElement> availableElements = new ArrayList<CapellaElement>(1);
		if (arch != null) {
	...
	public List<CapellaElement> getCurrentElements(CapellaElement element, boolean onlyGenerated) {
		...
		if (element instanceof StateTransition) {
			for (StateTransitionRealization link : ((StateTransition) element).getOwnedStateTransitionRealizations()) {
