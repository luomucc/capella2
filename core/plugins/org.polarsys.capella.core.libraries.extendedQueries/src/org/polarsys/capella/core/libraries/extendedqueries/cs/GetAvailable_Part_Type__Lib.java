	...
	public List<Object> execute(Object input, IQueryContext context) throws QueryException {
		...
		for (IModel library : libraries) {
			...
			for (BlockArchitecture currentBlockArchitecture : BlockArchitectureExt.getAllAllocatedArchitectures(blockArchitecture)) {
				...
				if (EcoreUtil2.getFirstContainer(in, CsPackage.Literals.COMPONENT_CONTEXT) != null) {
					result.addAll(QueryInterpretor.executeQuery(QueryIdentifierConstants.GET_SUB_DEFINED_ACTORS, currentBlockArchitecture, context));
				}
			}
		}
		return result;
