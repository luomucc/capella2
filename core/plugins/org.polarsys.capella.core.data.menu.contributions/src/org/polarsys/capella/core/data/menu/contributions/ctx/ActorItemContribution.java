...
public class ActorItemContribution implements IMDEMenuItemContribution {
  /**
  ...
  public Command executionContribution(final EditingDomain editingDomain, ModelElement containerElement, final ModelElement createdElement,
    ...
    if (createdElement instanceof Actor) {
      if (((PartitionableElement) createdElement).getRepresentingPartitions().size() == 0) {
        EObject ctxArch = EcoreUtil2.getFirstContainer(containerElement, CtxPackage.Literals.SYSTEM_ANALYSIS);
        if (ctxArch != null) {
          SystemContext context = ((SystemAnalysis) ctxArch).getOwnedSystemContext();
          if (context != null) {
