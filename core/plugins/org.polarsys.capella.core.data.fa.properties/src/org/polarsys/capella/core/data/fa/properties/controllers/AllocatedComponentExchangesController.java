...
public class AllocatedComponentExchangesController extends AbstractMultipleSemanticFieldController {
  ...
  protected IBusinessQuery getReadOpenValuesQuery(EObject semanticElement) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement.eClass(), FaPackage.Literals.COMPONENT_EXCHANGE_ALLOCATOR__OWNED_COMPONENT_EXCHANGE_ALLOCATIONS);
  }
  ...
  protected void doRemoveOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ...
    for (EObject obj : (List<EObject>) semanticElement.eGet(semanticFeature)) {
      if ((obj instanceof ComponentExchangeAllocation)
        && ((ComponentExchangeAllocation) obj).getTargetElement().equals(object))
      {
        linkToRemove = obj;
      }
    }
