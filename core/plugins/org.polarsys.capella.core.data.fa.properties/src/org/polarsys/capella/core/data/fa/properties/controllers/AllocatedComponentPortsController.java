...
/**
 */
public class AllocatedComponentPortsController extends AbstractMultipleSemanticFieldController {
  /**
   * {@inheritDoc}
   */
  @Override
  protected IBusinessQuery getReadOpenValuesQuery(EObject semanticElement) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement.eClass(), CsPackage.Literals.PHYSICAL_PORT__OWNED_COMPONENT_PORT_ALLOCATIONS);
  }
  ...
  public List<EObject> loadValues(EObject semanticElement, EStructuralFeature semanticFeature) {
    ...
    if (lst instanceof Collection<?>) {
      for (Object obj : (Collection<?>) lst) {
        if (obj instanceof ComponentPortAllocation) {
          values.add(((ComponentPortAllocation) obj).getTargetElement());
        }
      }
    }
  ...
  protected void doAddOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ComponentPortAllocation link = FaFactory.eINSTANCE.createComponentPortAllocation();
  ...
  protected void doRemoveOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ...
    for (EObject obj : (List<EObject>) semanticElement.eGet(semanticFeature)) {
      if ((obj instanceof ComponentPortAllocation)
        && ((ComponentPortAllocation) obj).getTargetElement().equals(object))
      {
        linkToRemove = obj;
      }
    }
