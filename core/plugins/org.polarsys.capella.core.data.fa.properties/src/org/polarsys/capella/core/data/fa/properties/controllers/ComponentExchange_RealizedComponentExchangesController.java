...
/**
 */
public class ComponentExchange_RealizedComponentExchangesController extends AbstractMultipleSemanticFieldController {
  /**
   * {@inheritDoc}
   */
  @Override
  protected IBusinessQuery getReadOpenValuesQuery(EObject semanticElement) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement.eClass(), FaPackage.Literals.COMPONENT_EXCHANGE__OWNED_COMPONENT_EXCHANGE_REALIZATIONS);
  }
  ...
  public List<EObject> loadValues(EObject semanticElement, EStructuralFeature semanticFeature) {
    ...
    if (lst instanceof Collection<?>) {
      for (Object obj : (Collection<?>) lst) {
        if (obj instanceof ComponentExchangeRealization) {
          values.add(((ComponentExchangeRealization) obj).getTargetElement());
        }
      }
    }
  ...
  protected void doAddOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ComponentExchangeRealization link = FaFactory.eINSTANCE.createComponentExchangeRealization();
  ...
  protected void doRemoveOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ...
    for (EObject obj : (List<EObject>) semanticElement.eGet(semanticFeature)) {
      if ((obj instanceof ComponentExchangeRealization)
        && ((ComponentExchangeRealization) obj).getTargetElement().equals(object))
      {
        linkToRemove = obj;
      }
    }
