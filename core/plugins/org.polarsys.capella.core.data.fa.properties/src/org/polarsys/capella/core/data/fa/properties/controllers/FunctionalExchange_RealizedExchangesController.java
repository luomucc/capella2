...
/**
 */
public class FunctionalExchange_RealizedExchangesController extends AbstractMultipleSemanticFieldController {
  /**
   * {@inheritDoc}
   */
  @Override
  protected IBusinessQuery getReadOpenValuesQuery(EObject semanticElement) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement.eClass(), FaPackage.Literals.FUNCTIONAL_EXCHANGE__OWNED_FUNCTIONAL_EXCHANGE_REALIZATIONS);
  }
  ...
  public List<EObject> loadValues(EObject semanticElement, EStructuralFeature semanticFeature) {
    ...
    if (lst instanceof Collection<?>) {
      for (Object obj : (Collection<?>) lst) {
        if (obj instanceof FunctionalExchangeRealization) {
          values.add(((FunctionalExchangeRealization) obj).getTargetElement());
        }
      }
    }
  ...
  protected void doAddOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    FunctionalExchangeRealization link = FaFactory.eINSTANCE.createFunctionalExchangeRealization();
  ...
  protected void doRemoveOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ...
    for (EObject obj : (List<EObject>) semanticElement.eGet(semanticFeature)) {
      if ((obj instanceof FunctionalExchangeRealization)
        && ((FunctionalExchangeRealization) obj).getTargetElement().equals(object))
      {
        linkToRemove = obj;
      }
    }
