...
/**
 */
public class AllocatedFunctionPortsController extends AbstractMultipleSemanticFieldController {
  /**
   * {@inheritDoc}
   */
  @Override
  protected IBusinessQuery getReadOpenValuesQuery(EObject semanticElement) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement.eClass(), InformationPackage.Literals.PORT__OWNED_PORT_ALLOCATIONS);
  }
    ...
    if (lst instanceof Collection<?>) {
      for (Object obj : (Collection<?>) lst) {
        if (obj instanceof PortAllocation) {
          values.add(((PortAllocation) obj).getTargetElement());
        }
      }
    }
  ...
  protected void doAddOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    PortAllocation link = InformationFactory.eINSTANCE.createPortAllocation();
  ...
  protected void doRemoveOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ...
    for (EObject obj : (List<EObject>) semanticElement.eGet(semanticFeature)) {
      if ((obj instanceof PortAllocation)
        && ((PortAllocation) obj).getTargetElement().equals(object))
      {
        linkToRemove = obj;
      }
    }
