...
public class AddTrace extends AbstractReadWriteCommand {
   ...
   */
  public void run() {
    Trace trace = TraceNameHelper.getNewTraceInstanceFromTraceName(_traceType);
    if (trace != null) {
      if (!_tgtElement.equals(_srcElement)) {
        ...
        if(_srcElement instanceof Namespace) {
          ...
          if (container != null) {
            ...
            MessageDialog.openWarning(Display.getCurrent().getActiveShell(), Messages.getString("AddTrace.warning"), Messages.getString("AddTrace.msg1")); //$NON-NLS-1$ //$NON-NLS-2$
            trace.destroy();
          }
        }
      } else {
        ...
        trace.destroy();
        MessageDialog.openWarning(Display.getCurrent().getActiveShell(), Messages.getString("AddTrace.warning"), Messages.getString("AddTrace.msg2")); //$NON-NLS-1$ //$NON-NLS-2$
      }
    }
