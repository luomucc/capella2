...
public class Role_AllocatedActivitiesController extends AbstractMultipleSemanticFieldController {
  ...
  protected IBusinessQuery getReadOpenValuesQuery(EObject semanticElement) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement.eClass(), OaPackage.Literals.ROLE__OWNED_ACTIVITY_ALLOCATIONS);
  }
  ...
  protected void doRemoveOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ...
    for (EObject obj : (List<EObject>) semanticElement.eGet(semanticFeature)) {
      if ((obj instanceof ActivityAllocation)
        && ((ActivityAllocation) obj).getTargetElement().equals(object))
      {
        linkToRemove = obj;
      }
    }
