...
/**
 */
public class OperationalCapability_InvolvedEntitiesController extends AbstractMultipleSemanticFieldController {
  /**
   * {@inheritDoc}
   */
  @Override
  protected IBusinessQuery getReadOpenValuesQuery(EObject semanticElement) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement.eClass(), OaPackage.Literals.OPERATIONAL_CAPABILITY__OWNED_ENTITY_OPERATIONAL_CAPABILITY_INVOLVEMENTS);
  }
  ...
  public List<EObject> loadValues(EObject semanticElement, EStructuralFeature semanticFeature) {
    ...
    if (lst instanceof Collection<?>) {
      for (Object obj : (Collection<?>) lst) {
        if (obj instanceof EntityOperationalCapabilityInvolvement) {
          values.add(((EntityOperationalCapabilityInvolvement) obj).getInvolved());
        }
      }
    }
  ...
  protected void doRemoveOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ...
    for (EObject obj : (List<EObject>) semanticElement.eGet(semanticFeature)) {
      if ((obj instanceof EntityOperationalCapabilityInvolvement)
        && ((EntityOperationalCapabilityInvolvement) obj).getInvolved().equals(object))
      {
        linkToRemove = obj;
      }
    }
