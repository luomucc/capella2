...
public class AbstractCapability_InvolvedFunctionalChainsController extends AbstractMultipleSemanticFieldController {
  ...
  protected IBusinessQuery getReadOpenValuesQuery(EObject semanticElement) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement.eClass(), InteractionPackage.Literals.ABSTRACT_CAPABILITY__OWNED_FUNCTIONAL_CHAIN_ABSTRACT_CAPABILITY_INVOLVEMENTS);
  }
  ...
  protected void doRemoveOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ...
    for (EObject obj : (List<EObject>) semanticElement.eGet(semanticFeature)) {
      if ((obj instanceof FunctionalChainAbstractCapabilityInvolvement)
        && ((FunctionalChainAbstractCapabilityInvolvement) obj).getInvolved().equals(object))
      {
        linkToRemove = obj;
      }
    }
