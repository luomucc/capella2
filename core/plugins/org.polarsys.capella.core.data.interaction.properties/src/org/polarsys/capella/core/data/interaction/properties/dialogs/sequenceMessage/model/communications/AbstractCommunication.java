...
/** This class represents a possible candidate to selection for the sequence message creation wizard between two components.
 * Three kinds of communication are defined :
 * - communication by Communication links (see class LinkCommunication),
 * - communication by interface (see class InterfaceCommunication),
 * - communication by an orphan EI (see class UndefinedCommunication).
 * 
 *  This class implements EObject for technical reason : sequence message creation wizard viex (see class SelectInvokedOperationView)
 *  is based on a generic view in capella (see SelectElementsDialog) whose selectionable elements must be EObject (these elements are
 *  prompted in a tree that is calculated according to EMF containement property). */
public abstract class AbstractCommunication implements EObject, CapellaElement {
	...
	public String toString() {
		...
		buffer.append("    exchangeItem : "+(exchangeItem != null ? exchangeItem.getName()+" "+exchangeItem.getExchangeMechanism() : "")+'\n'); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		buffer.append("    allocation   : "+(exchangeItemAllocation != null ? exchangeItemAllocation.getName()+" "+exchangeItemAllocation.getSendProtocol()+" "+exchangeItemAllocation.getReceiveProtocol():"")+'\n'); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
	...
	public EList<Requirement> getAppliedRequirements() {
		return getRepresentativeElement().getAppliedRequirements();
	}
}
