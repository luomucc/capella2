...
/**
 */
public class AbstractCapability_RealizedCapabilitiesController extends AbstractMultipleSemanticFieldController {
  /**
   * {@inheritDoc}
   */
  @Override
  protected IBusinessQuery getReadOpenValuesQuery(EObject semanticElement) {
    return BusinessQueriesProvider.getInstance().getContribution(semanticElement.eClass(), InteractionPackage.Literals.ABSTRACT_CAPABILITY__OWNED_ABSTRACT_CAPABILITY_REALIZATIONS);
  }
    ...
    if (lst instanceof Collection<?>) {
      for (Object obj : (Collection<?>) lst) {
        if (obj instanceof AbstractCapabilityRealization) {
          values.add(((AbstractCapabilityRealization) obj).getTargetElement());
        }
      }
    }
  ...
  protected void doRemoveOperationInWriteOpenValues(EObject semanticElement, EStructuralFeature semanticFeature, EObject object) {
    ...
    for (EObject obj : (List<EObject>) semanticElement.eGet(semanticFeature)) {
      if ((obj instanceof AbstractCapabilityRealization)
        && ((AbstractCapabilityRealization) obj).getTargetElement().equals(object))
      {
        linkToRemove = obj;
      }
    }
